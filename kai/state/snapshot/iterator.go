...
func (dl *diskLayer) AccountIterator(seek common.Hash) AccountIterator {
	...
	return &diskAccountIterator{
		layer: dl,
		it:    dl.diskdb.NewIterator(kvstore.SnapshotAccountPrefix, pos),
	}
}
...
func (it *diskAccountIterator) Next() bool {
	...
	for {
		...
		if len(it.it.Key()) == len(kvstore.SnapshotAccountPrefix)+common.HashLength {
			break
		}
	}
	return true
...
func (dl *diskLayer) StorageIterator(account common.Hash, seek common.Hash) (StorageIterator, bool) {
	...
	return &diskStorageIterator{
		layer:   dl,
		account: account,
		it:      dl.diskdb.NewIterator(append(kvstore.SnapshotStoragePrefix, account.Bytes()...), pos),
	}, false
}
...
func (it *diskStorageIterator) Next() bool {
	...
	for {
		...
		if len(it.it.Key()) == len(kvstore.SnapshotStoragePrefix)+common.HashLength+common.HashLength {
			break
		}
	}
	return true
