...
func wipeContent(db kaidb.KeyValueStore) error {
	if err := wipeKeyRange(db, "accounts", kvstore.SnapshotAccountPrefix, len(kvstore.SnapshotAccountPrefix)+common.HashLength); err != nil {
		return err
	}
	if err := wipeKeyRange(db, "storage", kvstore.SnapshotStoragePrefix, len(kvstore.SnapshotStoragePrefix)+2*common.HashLength); err != nil {
		return err
	}
	...
	end := common.CopyBytes(kvstore.SnapshotAccountPrefix)
	end[len(end)-1]++

	if err := db.Compact(kvstore.SnapshotAccountPrefix, end); err != nil {
		return err
	}
	log.Info("Compacting snapshot storage area ")
	end = common.CopyBytes(kvstore.SnapshotStoragePrefix)
	end[len(end)-1]++

	if err := db.Compact(kvstore.SnapshotStoragePrefix, end); err != nil {
		return err
	}
